...
class CarsVerificationModelTrainer:
    ...
    def get_data_loaders(self):
        ...
        self.train_loader, self.trainset = get_data_loader(
            dataset_name=self.data_config['train_data']['dataset_name'],
            dataset_parameters=self.data_config['train_data'][
                'dataset_params'],
        ...
        self.val_loader, self.valset = get_data_loader(
            dataset_name=self.data_config['val_data']['dataset_name'],
        ...
        if self.model_config['model_name'] == 'cars_model':
            self.model_config['model_parameters']['cars_model_parameters'][
                'num_classes'] = len(set(self.trainset.df['class'].values))

        self.easy_val_pairs = pd.read_csv(
            self.data_config['val_pairs_data']['easy_val'])
        self.normal_val_pairs = pd.read_csv(
            self.data_config['val_pairs_data']['normal_val'])
        self.hard_val_pairs = pd.read_csv(
            self.data_config['val_pairs_data']['hard_val'])

    def set_train_parameters(self):
        """ Method for determining training parameters """

        self.optimizer = get_optimizer(
            optim_name=self.train_config['optimizer']['name'],
            model=self.model.model,
            optim_parameters=self.train_config['optimizer']['parameters'])
        if self.train_config['criterion']['name'] == 'Focal':
            self.train_config['criterion']['parameters'][
                'device'] = self.model.device
            self.train_config['criterion']['parameters']['class_num'] = len(
                set(self.trainset.df['class'].values))
        self.criterion = get_loss_func(
            loss_name=self.train_config['criterion']['name'],
        ...
        if 'scheduler' in self.train_config:
            if self.train_config['scheduler']['name'] == 'OneCycle':
                self.train_config['scheduler']['parameters'][
                    'steps_per_epoch'] = len(self.train_loader)
                self.train_config['scheduler']['parameters']['epochs'] = \
                self.train_config['num_epochs']
            self.scheduler = get_scheduler(
                scheduler_name=self.train_config['scheduler']['name'],
    ...
    def get_model(self):
        ...
        self.model = get_model(model_name=self.model_config['model_name'],
                               model_parameters=self.model_config[
                                   'model_parameters'])
    ...
    def fit_epoch(self):
        ...
        for idx, (imgs, labels) in tqdm(enumerate(self.train_loader)):
            imgs, labels = imgs.to(self.model.device), labels.to(
                self.model.device)
    ...
    def calculate_cosine_similarity(self, embs_1, embs_2):
        ...
        for emb_1, emb_2 in zip(embs_1, embs_2):
            similarity.append(
                cosine_similarity(emb_1.reshape(1, -1), emb_2.reshape(1, -1))[
                    0][0])
    ...
    def eval_epoch(self):
        ...
        for imgs, labels, img_paths in tqdm(self.val_loader):
            ...
            for img_path, emb in zip(img_cars_model_parameterspaths, embs):
        ...
        for idx, row in tqdm(self.easy_val_pairs.iterrows()):
            try:
                ...
                continue
            sim = \
            cosine_similarity(emb_1.reshape(1, -1), emb_2.reshape(1, -1))[0][0]
    ...
    def save_model(self, epoch: int):
        ...
        model_path = os.path.join(self.model_config['model_save_path'],
                                  self.model_config['model_save_name'])
    ...
    def save_results_log(self, epoch: int, train_loss: float,
                         easy_roc_auc: float, normal_roc_auc: float,
                         hard_roc_auc: float):
        """ Method for logging model results """

        log_path = os.path.join(self.train_config['logs_save_path'],
                                f"{self.model_config['model_save_name']}.txt")
    ...
    def train(self):
        ...
        for epoch in range(1, self.train_config['num_epochs'] + 1):
            ...
            self.save_results_log(epoch=epoch, train_loss=train_loss,
                                  easy_roc_auc=easy_metrics['roc_auc'],
                                  normal_roc_auc=normal_metrics['roc_auc'],
