...
def torch_h_flip(p: float):
    """ Function for defining transforms.Random Horizontal Flip - Image
    Mirroring """
    return transforms.RandomHorizontalFlip(p)


def torch_rotation(p: float, degrees: int):
    """ Function for defining transforms.RandomRotation - random rotation of
    the image (-degrees, degrees) """
