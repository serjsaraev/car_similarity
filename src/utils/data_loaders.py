...
class CarsDataset(Dataset):
    ...
    def __init__(self, root_path: str, sample_type: str, df_path: str,
                 transform: dict, imgaug_cutout=False):
        ...
        self.class2label = {cls: idx for idx, cls in
                            enumerate(self.df['class'].unique())}
    ...
    def __imgaug(self, image: Image, p=0.3, nb_iterations=10, size=0.1):
        ...
        try:
            if random.random() < p:
                ...
                img_arr = iaa.Cutout(nb_iterations=nb_iterations,
                                     size=size).augment_image(img_arr)
    ...
    def __getitem__(self, idx):
        try:
            img = Image.open(os.path.join(self.root_path, self.df.iloc[
                idx].image_name)).convert('RGB')
        except:
...
def get_data_loader(dataset_name: str, dataset_parameters: dict,
